clc ;
clear all ;
[ConvertedData,ConvertVer,ChanNames,GroupNames,ci] = convertTDMS(0,'SEApositiondatasine.tdms') ;
...
tt = timerx(mask);
refp = reference(mask);
act = actualcurrent(mask);
plot(tt,refp,'r','linewidth',1) ;
hold on;
...
% hgexport(gcf,'SEA sine wave position.pdf',figure_property); %Set desired file name